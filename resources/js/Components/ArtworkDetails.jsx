        ...
        <div className="w-auto min-w-[1000px] max-w-[1200px] text-sm p-8 pt-0 flex items-start gap-5">
            <div class="w-full mx-auto flex flex-wrap">
                ...
                <div class="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
                    <h2 class="text-sm title-font text-gray-500 tracking-widest">{artwork.categoryName}</h2>
                    <h1 class="text-gray-900 text-3xl title-font font-medium mb-1">{artwork.name}</h1>
                    {artwork.productStatusName === 'For Sale' ? (
                        <h3 className="bg-green-100 text-green-500 px-2 py-1 rounded w-fit">{artwork.productStatusName}</h3>
                    ) : artwork.productStatusName === 'For Approval' ?  (
                        <h3 className="bg-blue-100 text-blue-500 px-2 py-1 rounded w-fit">{artwork.productStatusName}</h3>
                    ) : (
                        <h3 className="bg-red-100 text-red-500 px-2 py-1 rounded w-fit">{artwork.productStatusName}</h3>
                    )}
                    ...
                    <div className="flex items-center justify-between">
                        <div class="flex">
                            <span class="title-font font-medium text-2xl text-gray-900">{artwork.price}</span>
                        </div>
                        { role === 'admin' && artwork.productStatusName === 'For Approval' && (
